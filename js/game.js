  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (daySessions.length > 0 && (game.tracking.score || game.tracking.won)) {
      ...
      if (!anyScore && game.tracking.won) {
        ...
        if (wins > 0) stackHtml += `<span class="tooltip-win">W x${wins}</span> `;
        if (losses > 0) stackHtml += `<span class="tooltip-loss">L x${losses}</span>`;
        stackHtml += `</div>`;
        content += stackHtml;
      } else {
        // List Mode
...
function renderBadges(allGames) {
    ...
    // 1. ALL TIME RANK (Standard Gold/Silver/Bronze)
    ...
    // 2. PLAY MILESTONES (Every 5 up to 50)
    const p = game.plays || 0;
    let bestPlay = 0;
    for(let i=5; i<=50; i+=5) { if(p >= i) bestPlay = i; }
    if(bestPlay > 0) {
        createBadge("Veteran", `${bestPlay}+ Plays`, `lvl-${bestPlay}`);
    }

    // 3. WIN MILESTONES (Every 5 up to 50)
    if (game.sessions) {
        const wins = game.sessions.filter(s => s.won === true).length;
        let bestWin = 0;
        for(let i=5; i<=50; i+=5) { if(wins >= i) bestWin = i; }
        if(bestWin > 0) {
            createBadge("Victor", `${bestWin}+ Wins`, `lvl-${bestWin}`);
        }
    }
    ...
    // Find months where this game was #1
    ...
    if (champMonths.length > 0) {
        // Determine tier based on how many months won
        let count = champMonths.length;
        let tier = 5; // Default start
        if(count >= 3) tier = 15;
        if(count >= 5) tier = 25;
        if(count >= 10) tier = 45;
        
        // Pass tooltip text
        createBadge("Champion", `x${count} Months`, `lvl-${tier}`, `Months Won:<br>${champMonths.join("<br>")}`);
    }
...
function createBadge(title, sub, classes, tooltipText = null) {
    const el = document.createElement("div");
    el.className = `badge ${classes}`;
    
    let html = `<div class="badge-title">${title}</div><div class="badge-sub">${sub}</div>`;
    
    // Add tooltip if provided
    if (tooltipText) {
        html += `<div class="badge-tooltip">${tooltipText}</div>`;
    }
...
init();import { getGames, updateGame, deleteGame } from "./data.js";
